...
def app_surf_update(app_surf, app_surf_rect, destination, player_one, player_two, player_three, timer_text, current_turn):
    app_surf.fill('white')
    ...
    pygame.draw.circle(app_surf, player_three['colour'], player_three['pygame_coords'], radius=3, width=2)
    display_coordinates(app_surf, player_one['pygame_coords'], player_two['pygame_coords'], player_three['pygame_coords'], destination['pygame_coords'], timer_text, current_turn)
...
def display_coordinates(app_surf, player_one_coords, player_two_coords, player_three_coords, destination_coords, timer_text, current_turn):
    ...
    player_three_text = font.render(f'Player Three: {player_three_coords}', True, pygame.Color('black'))
    ...
    app_surf.blit(player_three_text, (20, 80))
    app_surf.blit(destination_text, (20, 110))
    app_surf.blit(timer, (20, 140))
    app_surf.blit(turn_text, (20, 170))

# Function to switch turns between players
def switch_turn(current_turn):
    return 1 if current_turn == 3 else current_turn + 1
...
while True:
    for event in pygame.event.get():
        ...
        move_npc()
        update_game_state()  # Update game state
        print("\nUpdated Game State after Player Three's Turn:")
        print("Player One's Information:")
        print_player_info(player_one)
        print("\nPlayer Two's Information:")
        print_player_info(player_two)
    ...
    # Update display
    app_surf_update(app_surf, app_surf_rect, destination, player_one, player_two, player_three, timer_text, current_turn)
    refresh_window()
